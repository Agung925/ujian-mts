...
<div class="max-w-3xl mx-auto">
    ...
    <nav class="text-xs text-gray-400 mb-4 flex items-center gap-1">
        <a href="{{ route('guru.ujian.index') }}" class="hover:text-gray-600">Ujian</a>
        <span>/</span>
        <a href="{{ route('guru.ujian.show', $ujian) }}" class="hover:text-gray-600">{{ $ujian->judul }}</a>
        <span>/</span>
        <span class="text-gray-600">{{ $sesi->siswa->name }}</span>
    </nav>

    {{-- ============================================================
         HEADER NILAI
    ============================================================ --}}
    <div class="bg-white rounded-2xl border border-gray-200 shadow p-6 mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
        <div class="flex-1">
            <h1 class="text-lg font-bold text-gray-800">{{ $sesi->siswa->name }}</h1>
        ...
        <div class="text-center sm:text-right shrink-0">
            <p class="text-xs text-gray-400 mb-0.5">Nilai Akhir</p>
    ...
    <div class="grid grid-cols-4 gap-3 mb-6 text-center">
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-3">
            <p class="text-xl font-bold text-gray-700">{{ $totalSoal }}</p>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-3">
            <p class="text-xl font-bold text-gray-400">{{ $totalKosong }}</p>
    ...
    <div class="space-y-4 mb-8">
        <h2 class="text-base font-semibold text-gray-700">Jawaban per Soal</h2>
        ...
        @php
            ...
            if ($jawaban === null) {
                ...
                $badgeKelas  = 'bg-gray-100 text-gray-500';
        ...
        <div class="bg-white rounded-xl border {{ $borderKelas }} p-5">
            {{-- Header --}}
            <div class="flex items-start justify-between gap-3 mb-3">
                <div class="flex items-center gap-2">
                    ...
                    @endif
                    <span class="text-xs text-gray-400">Bobot: {{ $soal->pivot->bobot_nilai }}</span>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    @php
                        ...
                        if ($iniBenar && $dipilihSiswa) {
                            ...
                            $kelas = 'bg-gray-50 border-gray-200 text-gray-600';
                        }
                    @endphp
            ...
            @else
                <div class="mb-3">
                    <p class="text-xs font-semibold text-gray-500 mb-1">Jawaban siswa:</p>
                    @if($jawaban?->jawaban_essay)
                        <div class="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 whitespace-pre-wrap">{{ $jawaban->jawaban_essay }}</div>
                    @else
    ...
    <div class="text-center">
        <a href="{{ route('guru.ujian.show', $ujian) }}"
           class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-6 py-2.5 rounded-xl text-sm transition">
