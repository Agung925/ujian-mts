...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Detail Soal</h1>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="flex flex-wrap gap-2 mb-5">
            ...
            {{-- Bobot nilai --}}
            <span class="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
        ...
        {{-- Teks pertanyaan --}}
        <div class="mb-5">
            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Pertanyaan</p>
            <p class="text-gray-800 dark:text-gray-100 leading-relaxed text-base">{{ $bankSoal->pertanyaan }}</p>
        </div>

        {{-- Gambar jika ada --}}
        @if($bankSoal->gambar)
            <div class="mb-5">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Gambar Soal</p>
                <img src="{{ $bankSoal->url_gambar }}" class="max-h-60 rounded-lg border border-gray-200 dark:border-gray-700">
            </div>
        @endif
        ...
        @if($bankSoal->tipe_soal === 'pg')
            <div class="mb-5">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Pilihan Jawaban</p>
                <div class="space-y-2">
                    @foreach($bankSoal->pilihanJawaban as $pilihan)
                        <div class="flex items-center gap-3 px-4 py-2 rounded-lg
                            {{ $pilihan->is_benar ? 'bg-green-50 border border-green-300' : 'bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700' }}">
                            <span class="font-bold w-6 flex-shrink-0
                                {{ $pilihan->is_benar ? 'text-green-700' : 'text-gray-500 dark:text-gray-400' }}">
                                {{ $pilihan->label }}
                            </span>
                            <span class="text-sm {{ $pilihan->is_benar ? 'text-green-800 font-medium' : 'text-gray-700 dark:text-gray-300' }}">
        ...
        @if($bankSoal->tipe_soal === 'bs')
            <div class="mb-5">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Jawaban yang Benar</p>
                @php $jawabanBenarBs = $bankSoal->jawabanBenar; @endphp
                <div class="flex gap-4">
                    <div class="px-6 py-3 rounded-lg border
                        {{ ($jawabanBenarBs?->label === 'Benar') ? 'bg-green-50 border-green-400' : 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700' }}">
                        <span class="font-medium {{ ($jawabanBenarBs?->label === 'Benar') ? 'text-green-700' : 'text-gray-500 dark:text-gray-400' }}">
                            ✓ Benar
                        </span>
                    </div>
                    <div class="px-6 py-3 rounded-lg border
                        {{ ($jawabanBenarBs?->label === 'Salah') ? 'bg-green-50 border-green-400' : 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700' }}">
                        <span class="font-medium {{ ($jawabanBenarBs?->label === 'Salah') ? 'text-green-700' : 'text-gray-500 dark:text-gray-400' }}">
                            ✗ Salah
                        </span>
                    </div>
                </div>
        ...
        @if($bankSoal->tipe_soal === 'essay' && $bankSoal->kunci_essay)
            <div class="mb-5 bg-orange-50 border border-orange-200 rounded-lg p-4">
                ...
                <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ $bankSoal->kunci_essay }}</p>
            </div>
        @endif

        {{-- Info tambahan --}}
        <div class="text-xs text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-4 mt-4">
